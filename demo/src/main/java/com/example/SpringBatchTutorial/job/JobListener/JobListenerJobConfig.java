 ...
 * run: --spring.batch.job.name=JobListener 를 편집인자에 넘김
 */
@Configuration
...
public class JobListenerJobConfig {
    ...
    public Job JobListener() {
        ...
        return new JobBuilder("JobListener", jobRepository)
                ...
                .start(jobListStep()) // 첫 번째로 실행할 Step 지정
                .build(); // Job 생성
    }
     ...
     */
    @Bean
    public Step jobListStep() {


        // [변경점 3] new StepBuilder("이름", jobRepository) 사용
        return new StepBuilder("jobListStep", jobRepository)
                ...
                .tasklet((contribution, chunkContext) -> {
                    //System.out.println("JobListener 등장");

                    // after 잡을 보고싶을경우 아래 throw를 주석하고 return 을 주석 해제하면 됩니다.
                    throw new Exception("테스트를 위해 강제로 발생시킨 에러입니다.");
...
public class JobListenerConfig {
    ...
    @Bean
    public Job JobListener() {
        return jobBuilderFactory.get("JobListener") // "JobListener"이라는 이름으로 Job 생성
                // RunIdIncrementer: Job 실행 시마다 파라미터 ID를 증가시켜, 같은 Job을 여러 번 재실행할 수 있게 함
                .incrementer(new RunIdIncrementer())
                .start(jobListStep()) // Job 시작 시 실행할 첫 번째 Step 지정
                .build(); // Job 빌드 및 반환
    }
    ...
    @Bean
    public Step jobListStep() {
        return stepBuilderFactory.get("jobListStep") // "jobListStep"이라는 이름으로 Step 생성
